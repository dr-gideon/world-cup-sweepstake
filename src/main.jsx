...
function ProviderPanel({ state, action }) {
  ...
  return <section className="admin-panel export-panel"><div className="admin-panel-title">Football data and Tele summaries</div><p className="admin-panel-sub">Football-Data sync is rate-limit-aware. It records remaining request headers and skips importing unmatched teams. LLM summaries are cached and fall back to local copy if no LLM key is configured.</p>
    ...
    <div className="provider-status"><b>Football-Data</b><span>{footballStatus ? `${footballStatus.status}: ${footballStatus.message}` : "Not synced yet"}</span><em>{footballStatus ? `Requests left: ${footballStatus.requestsAvailable ?? "?"}; reset: ${footballStatus.resetSeconds ?? "?"}s` : "Headers will show after sync"}</em></div><div className="provider-status"><b>Auto-sync</b><span>{state.scheduler?.enabled ? "Enabled" : "Disabled"}{state.scheduler?.running ? " · running" : ""}</span><em>{state.scheduler?.lastMessage || "Set FOOTBALL_DATA_AUTO_SYNC=1 to enable."}</em></div>
